...
class DataLoader:
    ...
    def get_header_index(self, field_name: str) -> int:
        """获取字段在CSV中的列索引"""
        if not self.headers:
            raise ValueError("Headers未加载，请先调用load_csv()")
        return self.headers.index(field_name)


    def get_value(self, row: np.ndarray, field_name: str, as_float: bool = False) -> Any:
        """根据列名获取行中对应值"""
        try:
    ...
    def get_building_data(self, row_index: int) -> Dict[str, Any]:
        ...
        # 2. 构造信息
        structure_info = {
            "building_category": self.get_value(row, "建筑类别"),
            "sub_category": self.get_value(row, "建筑子类"),
            "construction_grades": self.get_value(row, "建筑等级"),
            "roof_forms": self.get_value(row, "屋顶形式"),
            "ridge_types": self.get_value(row, "屋脊类型"),
        ...
        # 5.2 进深数据
        depth_total = np.array(self.get_value(row, "通进深", True))
